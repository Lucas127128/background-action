...
test('success', async () => {
  ...
  post.stdout.split('\n').forEach((line) => {
    if (line.includes('::group::Truncated Error Output:'))
      return (sawStdErrGroup = true);
    if (line.includes('::group::Truncated Output:'))
      return (sawStdOutGroup = true);
    ...
    if (line.includes(':std')) {
      ...
      if (!postPids[pid]) {
        postPids[pid] = { stdout: 0, stderr: 0 };
      }
      // output is in order
