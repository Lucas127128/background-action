...
function normalizeInputs(inputs) {
  ...
  if (
    logOutputIf &&
    /true|false|failure|exit-early|timeout|success/.test(logOutputIf) ==
      false
  ) {
    throw new Error(
  ...
  try {
    ...
    if (waitOn.resources.length === 0)
      throw new Error(
        'You must provide one or more resources, see: https://github.com/jeffbski/wait-on#readme',
      );
  }
...
export default normalizeInputs(getRawInputs());
